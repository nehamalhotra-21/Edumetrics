...
// ── FLAGGED CARDS (compact list-style) ──
function buildFlaggedCards(){
  ...
  FLAGGED.forEach((s,i)=>{
    ...
    card.innerHTML=`
      ...
      <div class="flag-reason" title="${reason}">${reason}</div>
      <div class="flag-btn-row">
        <button class="view-btn" style="background:${r.bg};color:${r.txt};border:1px solid ${r.border}" onclick="openDetail(${i})">View →</button>
      </div>`;
...
// ── LAST WEEK CARDS (compact list-style) ──
function buildLastWeekCards(){
  ...
  LAST_WEEK.forEach((s,i)=>{
    ...
    card.innerHTML=`
      ...
      <div class="flag-reason" title="${topFactor}">${topFactor}</div>
      <div class="flag-btn-row">
        <button class="view-btn" style="background:${r.bg};color:${r.txt};border:1px solid ${r.border}" onclick="openLwDetail(${i})">View →</button>
      </div>`;
...
let nextTodoId=10;
const CAL_EVENTS=[
  {day:0,start:9,end:10.5,title:"Data Structures Lecture",color:"rgba(88,166,255,0.85)",textColor:"#fff"},
  {day:0,start:14,end:15,title:"Office Hours",color:"rgba(63,185,80,0.75)",textColor:"#fff"},
  {day:1,start:11,end:12,title:"Algorithm Lab",color:"rgba(188,140,255,0.85)",textColor:"#fff"},
  {day:2,start:9,end:10.5,title:"Data Structures Lecture",color:"rgba(88,166,255,0.85)",textColor:"#fff"},
  {day:2,start:16,end:17,title:"Risk Review Meeting",color:"rgba(248,81,73,0.85)",textColor:"#fff"},
  {day:3,start:10,end:11.5,title:"Algorithm Lab",color:"rgba(188,140,255,0.85)",textColor:"#fff"},
  {day:4,start:9,end:10,title:"Faculty Meeting",color:"rgba(210,153,34,0.85)",textColor:"#fff"},
  {day:4,start:11,end:12,title:"Data Structures Lecture",color:"rgba(88,166,255,0.85)",textColor:"#fff"},
];
...
function setMidtermView(v){
  ...
  if(v==='pre')buildPreMidtermCharts();
  if(v==='post')buildPostMidtermCharts();
}
function setEndtermView(v){
  ...
  if(v==='pre')buildPreEndtermCharts();
  if(v==='post')buildPostEndtermCharts();
}
